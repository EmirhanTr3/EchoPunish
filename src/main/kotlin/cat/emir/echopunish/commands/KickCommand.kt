...
class KickCommand(plugin: EchoPunish) : EchoCommand<EchoPunish>(plugin) {
    override fun getCommand(): LiteralArgumentBuilder<CommandSourceStack> {
        return command("kick") {
            requires { it.sender.hasPermission("echopunish.kick") }
            argument("player", ArgumentTypes.player()) {
                punishmentReasonArgument(::execute, "You have been kicked.")
            }
        }
    }
    ...
    fun execute(ctx: CommandContext<CommandSourceStack>, silent: Boolean, reason: String): Int {
        ...
        val player = ctx.getPlayer("player") ?: return 1

        ifNotExempt(sender, player, Punishment.Type.KICK, "echopunish.kick.exempt") {
